This page describes in detail how to customize the runtime environment of PSOM pipelines. The only necessary option describes where to save the logs :
```matlab
>> opt.path_logs = '/home/pbellec/svn/psom/trunk/data_demo/';
```
All other options could be set by default and would execute the pipeline in the so-called 'batch' mode. By changing these options, it is possible to get full advantage of multiple CPUs or even distributed computational resources. 
